  ...
  try {
    ...
    if (!authResponse.ok) {
      ...
      return res.status(200).json({
        ...
        protectionCreated: false,
        message:
          "Refund protection service unavailable: Unable to communicate with auth token endpoint",
      });
    }
    ...
    try {
      const email = teakOrderPayload.customer.email;
      const orderNumber = teakOrderPayload.order_number;

      const metadataResponse = await fetch(
        `${API_URL}/orders/${orderNumber}/metadata?email=${encodeURIComponent(email)}`,
        {
          method: "GET",
          headers: {
            Authorization: `JWT ${authData.token}`,
